...
> ⚠️ Multiple copies exist under profile dirs — always use the canonical path above.
...
- **⚠️ China firewall**: OSRM (`router.project-osrm.org`) and Nominatim often
  timeout from Chinese servers. Both `distance` and `directions` will hang
  and return nothing. The `search` command may also fail.

## China Fallback: Distance Without OSRM

When OSRM is unreachable (China), use `web_search` to get driving distances
between two named places. Query pattern:

```
"{origin} 到 {destination} 驾车距离 时间"
```

Combine search results with geographic knowledge to sort locations by
distance and estimate travel times. Present results with approximate
ranges (e.g., "约14km，驾车~30min"). This is accurate enough for planning
and avoids the 15s timeout loop.
