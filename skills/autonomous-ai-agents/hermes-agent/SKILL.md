...
### Vision for non-vision models

When the main model lacks vision (e.g. DeepSeek), configure a Chinese vision provider as auxiliary.
See `references/chinese-vision-providers.md` for GLM/DashScope/Kimi setup with free tiers.
...
The `browser` toolset provides full browser automation: navigate, click, type, scroll, screenshot, vision analysis, and JS console execution. It's powerful but has sharp edges — especially with Chinese websites. See `references/browser-automation-pitfalls.md` for:

- **Chinese site blocking**: Baidu throws captchas, Bing CN returns empty pages
- **Vision hallucination**: `browser_vision` can confidently misidentify page content; use `browser_console` to extract text reliably
- **Reliability hierarchy**: snapshot > console > vision for content extraction
...
For cost-conscious patterns (no_agent watchdog scripts, memory tradeoffs, Chinese mirror installs):
`references/cost-conscious-cron.md`

For model selection and pricing to avoid blowing student budgets:
`references/deepseek-pricing-guide.md`

For designing cron prompts that produce useful output instead of empty results:
`references/cron-prompt-design.md`
...
### ⚠️ Before switching provider/model: ALWAYS test the key FIRST

Pitfall: Changing model.provider + restarting gateway without verifying the API key has credits causes a 402 retry loop — the gateway keeps retrying failed API calls, user sees repeated garbage replies, and you can't even respond.

**Safe switching procedure:**
1. **Test key with curl** — never switch without verification:
   ```bash
   ...
   # Z.AI:      curl -s https://open.bigmodel.cn/api/paas/v4/models -H "Authorization: Bearer $ZAI_API_KEY"
   ```
2. If 402/401 → stop. Tell user the key has no credits BEFORE switching.
3. If 200 → `hermes config set model.provider <provider>` + `hermes config set model.default <model>`
4. `hermes gateway restart`
5. Verify next turn works

Keep the current working provider's key alongside the new one — don't overwrite it. That way a failed switch can be reverted without digging up old credentials. For budget-constrained users: see `references/deepseek-pricing-guide.md`.

### Fallback model configuration

When setting up a fallback chain (provider fails → auto-switch), `fallback_model` must be at **root level** of config.yaml (not under `model:`). Wrong placement means the fallback is silently ignored. See `references/fallback-model-config.md` for the exact format and how to verify it works.
...
- **Chinese search engines hijack results:** Baidu, Bing CN, Sogou often return captchas or unrelated sponsored results (e.g. searching "瑞幸" returns "故宫" links). For browser-based search demos, use `bing.com` (international) or `google.com` directly. For actual search, the `web_search` tool (Tavily backend) works reliably — don't confuse browser search demos with the built-in search tool.
...
### Chinese vision providers (free/low-cost)

When DeepSeek is the main model (no vision), configure a domestic Chinese provider for image analysis. All three work within China without a VPN:

| Provider | Config Key | Model | Cost | Signup |
|----------|-----------|-------|------|--------|
| **Z.AI / 智谱 GLM** | `ZAI_API_KEY` | `glm-4v-flash` | Free tier (~18元 new user) | bigmodel.cn |
| **DashScope / 阿里** | `DASHSCOPE_API_KEY` | `qwen-vl-plus` | Free tier | dashscope.aliyun.com |
| **Kimi / Moonshot** | `KIMI_API_KEY` | `kimi-vision` | Free credits | platform.moonshot.cn |

**Complete setup (recommended: 智谱 GLM):**

```bash
# 1. Add API key to .env
echo "ZAI_API_KEY=your-key-here" >> ~/.hermes/profiles/<profile>/.env

# 2. Configure vision auxiliary
hermes config set auxiliary.vision.provider zai
hermes config set auxiliary.vision.model glm-4v-flash

# 3. (Optional) Register provider explicitly
...
# 4. Restart gateway for env changes
hermes gateway restart
```

**Also set as fallback model (DeepSeek fails → auto-switch):**
```bash
hermes config set model.fallback_providers "['zai']"
hermes config set model.fallback_model "glm-5"
```

**Verify:** send an image via WeChat and check gateway logs — vision_analyze should now return Chinese text.

**Pitfall:** After changing `.env`, the gateway process must be restarted to pick up new environment variables. `hermes config set` changes take effect on next session, but `.env` changes need a process restart.

**For Chinese users without access to Google/OpenRouter:** Chinese providers (Z.AI/GLM, DashScope, Kimi, MiniMax) offer free vision tiers. See `references/chinese-vision-providers.md` for the step-by-step setup with free API keys.

**For Chinese users**: when the primary model lacks vision (e.g. DeepSeek), configure a domestic vision provider like 智谱 GLM. See `references/china-vision-fallback.md` for the quick setup.

**Common pattern — DeepSeek primary + Claude vision fallback:**
```bash
...
After config, `vision_analyze` auto-routes images to the vision provider. Check with:
```bash
hermes config | grep -A3 auxiliary.vision
```
Restart session (`/reset`) after config changes.

**Why this matters:** Non-vision primary models (DeepSeek, many local models) will fail `vision_analyze` with `"unknown variant 'image_url'"`. Don't try to install OCR packages — configure the vision fallback instead. Chinese OCR packages (EasyOCR, CnOCR, PaddleOCR) all require downloading multi-hundred-MB models that are often blocked from CN servers anyway.
