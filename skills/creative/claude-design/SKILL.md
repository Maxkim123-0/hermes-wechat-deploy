...
6b. **Render to high-res PNG (chat/WeChat delivery)**
   - When the artifact must be shown in a chat interface, DO NOT use browser screenshot tools — they are limited to viewport resolution and produce blurry, low-quality images.
   - Instead, render the HTML to a crisp 4K PNG via Playwright headless Chromium:
     ```bash
     NODE_PATH=/usr/local/lib/node_modules node scripts/render-html.js input.html output.png 2
     ```
   - The script renders at 1920×1080 viewport with 2x deviceScaleFactor → 3840×2160 (4K) output.
   - Delivery: `MEDIA:/path/to/output.png` in the chat response.
   - Requirements: `npm install -g playwright && npx playwright install chromium`
   - The `scripts/render-html.js` script is stateless and reusable across all HTML design artifacts.
...
**CRITICAL — Never fabricate data sources.** Do not invent URLs, BV numbers, API endpoints, DOIs, paper citations, view counts, or any identifier that looks verifiable. If the user asks for a B站 video link, search B站 and extract real BV numbers — never guess or make them up. If a source cannot be verified, say so explicitly rather than producing a plausible-looking fake. The user considers this academic dishonesty and has zero tolerance for it.
...
**Embedded media** (iframes, video, audio):

- Check all `src` attributes: `//` protocol-relative URLs **will break** when the HTML is opened from `file://` — the browser resolves `//player.bilibili.com/...` to `file://player.bilibili.com/...`. Always use explicit `https://` for iframe sources in portable HTML files.
- Headless browsers typically lack HTML5 video codecs. A blank or white iframe in a headless test does NOT mean the video is broken — it means the test environment can't decode video. Verify embedded players by:
  1. `curl -sI` the player URL to confirm HTTP 200 and `access-control-allow-origin: *`
  2. Navigate the browser directly to the video page to confirm the BV/ID resolves
  3. Check the browser console for player framework init messages (e.g., B站's `bili-fe-fp`, `白屏检测是否正常 {status: "ok"}`)
  4. **Tell the user what was verified and what couldn't be verified in the test environment.**
- For B站 specifically, the standard embed pattern is: `https://player.bilibili.com/player.html?bvid=...&page=1&high_quality=1&autoplay=0` with `referrerpolicy="no-referrer"` and `sandbox="allow-scripts allow-same-origin allow-popups allow-presentation"`. See `references/bilibili-embed.md` for full details.
...
- Do not deliver design artifacts as browser screenshots. They are low-resolution viewport caps that users will correctly call unacceptable. Use the Playwright 4K rendering pipeline instead (see step 6b and `scripts/render-html.js`).
- Do not claim browser verification unless it actually happened.
- Do not use `//` protocol-relative URLs in standalone HTML files meant to be opened from disk (`file://`). The browser resolves `//example.com` to `file://example.com`, breaking all embedded iframes, images, and scripts. Always use explicit `https://`.
- Do not trust a blank iframe in a headless browser as proof the media is broken — headless Chrome lacks video codecs. Verify embedded players with `curl -sI` and browser console checks instead.
